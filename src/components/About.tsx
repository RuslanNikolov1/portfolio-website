...
const About = memo(() => {
  ...
  const [currentHobbyCard, setCurrentHobbyCard] = useState(0);

  const HOBBY_SLIDES_COUNT = 4;

  const goToHobbyCard = useCallback((targetIndex: number) => {
    const normalizedIndex = (targetIndex + HOBBY_SLIDES_COUNT) % HOBBY_SLIDES_COUNT;
    setCurrentHobbyCard(normalizedIndex);
  }, []);

  const handleHobbyDragEnd = useCallback((_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    if (info.offset.x < -60) {
      goToHobbyCard(currentHobbyCard + 1);
      return;
    }
  ...
  }, [currentHobbyCard, goToHobbyCard]);
    ...
    <section id="about" className={styles.about} ref={aboutRef}>
      ...
      <div className={styles.overlay}></div>
        ...
        <div className={styles.hobbiesCarousel}>
          <div className={styles.hobbiesViewport}>
            <motion.div
              className={styles.hobbiesTrack}
              animate={{ x: `-${currentHobbyCard * 100}%` }}
            ...
            >
              <div className={styles.hobbySlide}>
                <div className={`${styles.sportsContainer} ${styles.hobbyCard}`}>
                  <button
                    type="button"
                    className={`${styles.hobbyNavButton} ${styles.hobbyNavButtonLeft}`}
                    onClick={() => goToHobbyCard(currentHobbyCard - 1)}
                    aria-label="Show previous hobby card"
                  >
                    <ChevronLeft size={22} />
                  </button>
                  <button
                    type="button"
                    className={`${styles.hobbyNavButton} ${styles.hobbyNavButtonRight}`}
                    onClick={() => goToHobbyCard(currentHobbyCard + 1)}
                    aria-label="Show next hobby card"
                  >
              ...
              <div className={styles.hobbySlide}>
                <div className={`${styles.artContainer} ${styles.hobbyCard}`}>
                  <button
                    type="button"
                    className={`${styles.hobbyNavButton} ${styles.hobbyNavButtonLeft}`}
                    onClick={() => goToHobbyCard(currentHobbyCard - 1)}
